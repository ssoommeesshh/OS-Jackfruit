 ...
 *   - mutex-protected access to the list
 *   - periodic timer callback for soft/hard limit enforcement
 *   - ioctl interface: MONITOR_REGISTER / MONITOR_UNREGISTER
 */
...
struct monitored_entry {
    ...
    int            soft_warned;         /* 1 = warning already emitted */
 ...
 *
 * We use a mutex (not a spinlock) because the ioctl path runs in
 * process context and can sleep.  The timer callback also runs in
 * softirq context but we use mutex_trylock() there to avoid
 * blocking — a spinlock would be fine too, but a mutex keeps the
 * timer path non-blocking and the ioctl path simple.
 * ============================================================== */
                         ...
                         long rss_bytes)
{
    ...
    printk(KERN_WARNING
           ...
           "rss=%ld limit=%lu — sending SIGKILL\n",
           container_id, pid, rss_bytes, limit_bytes);
}
...
static void timer_callback(struct timer_list *t)
{
    ...
    /*
     * Use mutex_trylock so we never block inside the timer.
     * If the lock is held (e.g. an ioctl is in progress) we just
                          ...
                          unsigned long arg)
{
    ...
    /* ----------------------------------------------------------------
     * TODO 4: Register a new container process
     * ---------------------------------------------------------------- */
    if (cmd == MONITOR_REGISTER) {
        struct monitored_entry *entry;

        /* Basic sanity check on limits */
    ...
    /* ----------------------------------------------------------------
     * TODO 5: Unregister a container process
     * ---------------------------------------------------------------- */
    printk(KERN_INFO
...
static void __exit monitor_exit(void)
{
     ...
     * ============================================================== */
    {
        ...
        list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
            printk(KERN_INFO
                   "[container_monitor] Freeing entry: container=%s "
                   "pid=%d on module unload\n",
                   entry->container_id, entry->pid);
            list_del(&entry->list);
            kfree(entry);
        }
